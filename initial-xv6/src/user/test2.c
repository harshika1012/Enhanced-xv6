...
void test0(){
    ...
    if (fork() == 0) {
        ...
        sleep(1);
        usless_work();
        printf("Child 3 exited PID %d\n", getpid());
        exit(0);
    }
    if (fork() == 0) {
        settickets(prog4_tickets);
        printf("Child 4 started PID %d\n", getpid());
        sleep(1);
        usless_work();
        printf("Child 4 exited PID %d\n", getpid());
        exit(0);
    }
    wait(0);
    wait(0);
